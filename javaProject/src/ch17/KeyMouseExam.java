...
public class KeyMouseExam extends JApplet implements MouseListener, MouseMotionListener,KeyListener{
	...
	public void keyPressed(KeyEvent e) {
		...
		//max(값,값) 또는 min(값,값)는 앞에값과 뒤의값 중 최대값 최소값처리를 해주어야
		//이미지가 프레임 바깥으로 안빠져나감
		switch (e.getKeyCode()) {//키코드값에 따라 분기
		case KeyEvent.VK_UP://위쪽 키를 누르면
			y=Math.max(0, y-5); break;
		case KeyEvent.VK_DOWN://아래키를 누르면
			y=Math.min(500-height, y+5); break;
		case KeyEvent.VK_LEFT://왼쪽키를 누르면
			y=Math.min(0, x-5); break;
		case KeyEvent.VK_RIGHT://오른쪽키를 누르면
			y=Math.min(500-width,x+5); break;
		}
		repaint();//그래픽 갱신 요청
	}
	...
	public void mouseDragged(MouseEvent e) {
		//컴퍼넌트상에서 mouse버튼을 누른상태에서 드래그 했을때 호출
	...
	public void mouseClicked(MouseEvent e) {
		flag=true;//클릭할때 true처리
		//마우스 클릭한 좌표저장
		x=e.getX();//x값 저장
		y=e.getY();//y좌표값 저장
		//그래픽 갱신 요청 > paint를 자동호출
		repaint();
	}
	...
	public void mouseReleased(MouseEvent e) {
		//마우스 버튼을 완전히 놓았을 때 호출
	}

	@Override
	public void mouseEntered(MouseEvent e) {
		//마우스가 컴퍼넌트에 들어 갔을 때 호출
	}
